...
(function () {
	...
	$(function () {
		...
		function setSwiperNavPreview(swiper){
			...
			slidesCount = swiper.slides.filter(function(inx,el){return !el.classList.contains('swiper-slide-duplicate')}).length;
		...
		if (plugins.swiper.length) {
			for (let i = 0; i < plugins.swiper.length; i++) {
				let
					...
					pagination = node.querySelector('.swiper-pagination'),
					...
					if (markerItems.length){
						...
						for (var j = 0; j < markerItems.length; j++){
							...
							getLatLngObject(markerElement.getAttribute("data-location"), markerElement, plugins.maps[i], function(location, markerElement, mapElement){
								...
								var infoImg = markerElement.getAttribute("data-img");
								var contentString = '<div class="map-info-window">'+ (infoImg ? `<img class='map-info-img' src="${infoImg}" alt> ` : '') + `<div class="map-info-text">${info}</div>` + '</div>';
